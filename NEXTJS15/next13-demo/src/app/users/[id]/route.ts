...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  // If the user is not found, this will return 'null' in JSON format.
    ...
    return new Response(JSON.stringify({
      ...
      requestType: request,
      requestObject: [
        request.arrayBuffer, request.blob, request.formData, request.json, request.text,request.headers,request.method,request.redirect,request.url,request.bodyUsed,request.body,request.cache,request.credentials,request.destination,request.integrity,request.keepalive,request.mode,request.redirect,request.referrer,request.referrerPolicy,request.signal,request.headers,request.url,request.clone,request.arrayBuffer
      ],
    }), { status: 404 });
  }
